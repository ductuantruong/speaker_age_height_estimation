...
class DSODataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        file = self.files[idx]
        utt_id = int(file.split('.')[0])
        id = self.df[(self.df['Language'] == self.language) & (self.df['utt_begin_id'] <= utt_id) & (self.df['utt_end_id'] >= utt_id)].index
        ...
        return wav, torch.FloatTensor([height]), torch.FloatTensor([age]), torch.FloatTensor([gender]), id.values.tolist()[0]

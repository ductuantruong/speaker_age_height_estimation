...
def collate_fn(batch):
    (seq, height, r_height, age, r_age, gender, speaker_id) = zip(*batch)
    seql = [x.reshape(-1,) for x in seq]
    seq_length = [x.shape[0] for x in seql]
    data = rnn_utils.pad_sequence(seql, batch_first=True, padding_value=0)
    return data, height, r_height, age, r_age, gender, seq_length, speaker_id
...
if __name__ == "__main__":
    ...
    if hparams.model_checkpoint:
        ...
        for batch in tqdm(testloader):
            x, y_h, y_h_r, y_a, y_a_r, y_g, x_len, batch_speaker_id = batch
            x = x.to(device)
        ...
        df.to_csv('cls_reg_err_distr.csv')

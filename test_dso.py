...
def collate_fn(batch):
    (seq, height, age, gender, speaker_id) = zip(*batch)
    seql = [x.reshape(-1,) for x in seq]
    seq_length = [x.shape[0] for x in seql]
    data = rnn_utils.pad_sequence(seql, batch_first=True, padding_value=0)
    return data, height, age, gender, seq_length, speaker_id
...
if __name__ == "__main__":
    ...
    test_csv_path = hparams.test_speaker_csv_path
    ...
    list_speaker_id = test_df.index.values.tolist()
    ...
    if hparams.model_checkpoint:
        ...
        for test_lang in list_lang:
            ...
            for batch in tqdm(testloader):
                x, y_h, y_a, y_g, x_len, batch_speaker_id = batch
                x = x.to(device)
        ...
        for speaker_id in list_speaker_id:
            print(speaker_age_pred_dict[speaker_id])
            ...
            test_df.at[speaker_id, 'age_prediction'] = round(speaker_age_pred_dict[speaker_id], 2)
            test_df.at[speaker_id, 'height_prediction'] = round(speaker_height_pred_dict[speaker_id], 2)
            test_df.to_csv('dso_test.csv')
    else:

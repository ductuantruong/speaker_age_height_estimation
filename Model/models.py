...
class Wav2vec2BiEncoder(nn.Module):
    def __init__(self, upstream_model='wav2vec2',num_layers=6, feature_dim=768):
        ...
        self.mfcc_cnn = nn.Sequential(
            nn.Dropout(p=0.1),
            nn.Conv1d(in_channels=39, out_channels=768, kernel_size=1),
            nn.ReLU(),
            nn.BatchNorm1d(768, momentum=0.1, affine=True),
    ...
    def forward(self, x, x_len):
        ...
        x_mfcc = self.mfcc_extractor(x_input)["last_hidden_state"].transpose(1, 2)
        x_mfcc = self.mfcc_cnn(x_mfcc).transpose(1, 2)
        x = torch.cat((x, x_mfcc), dim=1)

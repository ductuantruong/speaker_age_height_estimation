...
class UncertaintyLossAge(Module):
    def __init__(self):
        super(UncertaintyLossAge, self).__init__()
        self.loss = None
        self.loss_age = None
        self.loss_gender = None
        self.log_var_age = Parameter(torch.tensor(0, requires_grad=True, dtype=torch.float32, device=device))
    ...
    def forward(self, input, target):
        ...
        self.loss_age = self.rmse_loss(input=age_pred, target=age_target)
        self.loss_age_var = torch.exp(-self.log_var_age) * self.loss_age + self.log_var_age
...
class UncertaintyLossHeight(Module):
    def __init__(self):
        super(UncertaintyLossHeight, self).__init__()
        self.loss = None
        self.loss_height = None
        self.loss_gender = None
        self.log_var_height = Parameter(torch.tensor(0, requires_grad=True, dtype=torch.float32, device=device))
    ...
    def forward(self, input, target):
        ...
        self.loss_height = self.rmse_loss(input=height_pred, target=height_target)
        self.loss_height_var = torch.exp(-self.log_var_height) * self.loss_height + self.log_var_height

        self.loss = self.loss_gender + self.loss_height

        self.loss_var = self.loss_gender_var + self.loss_height_var

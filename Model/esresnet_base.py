...
class ResNetWithAttention(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.age_fc = torch.nn.Linear(
            in_features=self.fc.in_features,
            out_features=1,
        )
        
        self.height_fc = torch.nn.Linear(
            in_features=self.fc.in_features,
            out_features=1,
        )
        
        self.gender_fc = torch.nn.Sequential(
            torch.nn.Linear(
                in_features=self.fc.in_features,
                out_features=1
            ),
    ...
    def _forward_classifier(self, x: torch.Tensor) -> torch.Tensor:
        y_age = self.age_fc(x)
        y_height = self.height_fc(x)
        y_gender = self.gender_fc(x)

        return y_age, y_height, y_gender
    ...
    def forward(self,
        ...
        y_age, y_height, y_gender = self._forward_classifier(x)
